...
static int refresh_locale_list() {
	...
	if (hkl_size > 0) {
		...
		HKL *hkl_list = static_cast<HKL *>(malloc(sizeof(HKL) * hkl_size));
				...
				if (hkl_list[i] != NULL) {
					...
					if (get_keyboard_layout_file(layoutFile, MAX_PATH) == UIOHOOK_SUCCESS) {
						...
						if (GetSystemDirectory(systemDirectory, MAX_PATH) != 0) {
							...
							locale_item = static_cast<KeyboardLocale *>(malloc(sizeof(KeyboardLocale)));
							...
							if (pKbdLayerDescriptor != NULL) {
								PKBDTABLES pKbd = static_cast<PKBDTABLES>(pKbdLayerDescriptor());
