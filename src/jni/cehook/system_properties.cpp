...
static BOOL CALLBACK monitor_enum_proc(HMONITOR hMonitor, HDC hdcMonitor, LPRECT lprcMonitor, LPARAM dwData) {
	...
	if (width > 0 && height > 0) {
		...
		screens->data[screens->count++] = {
				// Should monitor count start @ zero? Currently it starts at 1.
				screens->count,
				static_cast<int16_t>(origin_x),
				static_cast<int16_t>(origin_y),
...
//
////            MessageBox(0, "SETTING HOOK", "MessageBox caption", MB_OK);
////            hInst = GetModuleHandle(NULL);
////            SetWindowsHookEx(WH_KEYBOARD_LL, keyboard_hook_event_proc2, hInst, 0);
//
//			break;
//        }
//
