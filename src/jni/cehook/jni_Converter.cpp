	...
	if (javaType != NULL) {
		switch (nativeType) {
			...
			case EVENT_KEY_RELEASED:
				...
				*javaType = org_iceterm_cehook_keyboard_NativeKeyEvent_NATIVE_KEY_FIRST + (nativeType - 3);
				status = JNI_OK;
				break;
		...
		switch (javaType) {
			...
			case org_iceterm_cehook_keyboard_NativeKeyEvent_NATIVE_KEY_RELEASED:
				*nativeType = static_cast<event_type>((javaType + 3) -
                                                      org_iceterm_cehook_keyboard_NativeKeyEvent_NATIVE_KEY_FIRST);
				status = JNI_OK;
				break;
...
jint jni_ConvertToJavaLocation(unsigned short int *nativeKeyCode, jint *javaKeyLocation) {
	...
	if (nativeKeyCode != NULL && javaKeyLocation != NULL) {
		switch (*nativeKeyCode) {
			...
			case VC_ALT_L:
			case VC_META_L:
				*javaKeyLocation = org_iceterm_cehook_keyboard_NativeKeyEvent_LOCATION_LEFT;
				break;
			...
			case VC_ALT_R:
				...
				*javaKeyLocation = org_iceterm_cehook_keyboard_NativeKeyEvent_LOCATION_RIGHT;
				break;
			...
			case VC_KP_SEPARATOR:
				*javaKeyLocation = org_iceterm_cehook_keyboard_NativeKeyEvent_LOCATION_NUMPAD;
				break;
			...
			default:
				*javaKeyLocation = org_iceterm_cehook_keyboard_NativeKeyEvent_LOCATION_STANDARD;
				break;
		}
